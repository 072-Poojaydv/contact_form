...
class Query(db.Model):
    ...
    def __repr__(self):
        return f"Query('{self.name}' , '{self.email}' , '{self.message}')"
...
def contact():
    if request.method == "POST":
        ...
        u=Query(name=Name, email=Email, message=Message)
        db.session.add(u)
        db.session.commit()
        if app.config['ADMIN']:
            send_mail(app.config['ADMIN'],'New Query','mail/query',name=Name,email=Email, message=Message)
        flash('YOUR QUERY HAS BEEN SENT WILL CONTACT YOU SHORTLY!!!!',category='alert')
    return render_template("contact.html")
